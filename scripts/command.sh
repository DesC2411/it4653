...
# ──────────────────────────────────────────────────────────────────────────────
# 6) GAN part — everything lives in 5_gan.py, pick the stage with the first argument.
#
#    prepare : builds the FID ruler shared by the whole group (feature extractor +
#              the 5000 real reference images) and writes it to shared/. RUN ONCE,
#              then commit shared/. Every FID in this project is measured against
#              those two files; if each member builds their own, the numbers stop
#              being comparable and nothing warns you. Skip if shared/ already has
#              both files.
#    dcgan   : requirement 1 (architecture, loss curves) and 4 (instability).
#              --epochs must match the AE/VAE runs above — that is what "same epoch
#              budget" in the assignment means. Produces Hình 1-4 and dcgan.pt.
#    cgan    : requirement 3. Prints the share of samples a classifier reads back
#              as the digit that was requested. --digits picks the grid rows.
#    compare : requirement 2. Loads the two checkpoints plus every .npz under
#              --team-dir, then writes the FID table, the grids and the bar chart.
#              Files whose name contains "recon" are reported separately:
#              reconstruction FID is low by construction and does not belong next
#              to a GAN.
#    all     : the four stages in order, skipping prepare when shared/ is ready.
# ──────────────────────────────────────────────────────────────────────────────
uv run python 5_gan.py all --out-dir runs --epochs 10

# Or one stage at a time:
uv run python 5_gan.py prepare --shared-dir shared --fid-n 5000
uv run python 5_gan.py dcgan   --out-dir runs --epochs 10
uv run python 5_gan.py cgan    --out-dir runs --epochs 10
uv run python 5_gan.py compare --out-dir runs --shared-dir shared --team-dir shared

# Spell a date with the conditional generator instead of the default 0..9 grid:
uv run python 5_gan.py cgan --out-dir runs --epochs 10 --digits 2 0 2 6
